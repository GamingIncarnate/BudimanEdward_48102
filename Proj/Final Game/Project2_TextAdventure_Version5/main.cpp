...
void fillAry(int,int[][COLUMNS]);
void useAry(int,int[][COLUMNS]);
void inventory(int,int,int[],string[]);
bool death(int);
int load();
int dangerRd();
float ageNum(short);
...
int main(int argc, char** argv) {
    ...
    bool isDead=false;
    char menuItm;
    const int SIZE=100, INVSIZE=10;
    int instance=0, invNum[INVSIZE]={};
    float age;
    string storyItm[SIZE];
    string invName[INVSIZE];
    
    //Set random array
    srand(static_cast<unsigned int>(time(0)));
    int random[SIZE][COLUMNS];
    fillAry(SIZE,random);
    useAry(SIZE,random);
